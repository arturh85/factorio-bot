...
use crate::experiment::manifest::{Manifest, FirstRocketManifest, ResolvedFirstRocketManifest, expand_first_rocket_trials};
use crate::experiment::report::{compare, write_report, write_first_rocket_report};
use crate::experiment::runner::{run_first_rocket_experiment, RunnerConfig};
...
fn run(args: &ArgMatches, _context: &mut Context) -> Result<()> {
    match args.subcommand() {
        ...
        None => Err(miette!("experiment requires a subcommand (prepare|run|report)")),
    }
}
...
fn run_trials(args: &ArgMatches) -> Result<()> {
    ...
    if let Ok(rocket_manifest) = factorio_bot_core::serde_json::from_str::<ResolvedFirstRocketManifest>(&manifest_json) {
        if dry_run {
            println!("DRY RUN: would execute {} trials from first-rocket manifest", 
            ...
            println!("  Trial wall seconds: {}", rocket_manifest.trial_wall_seconds);
        ...
        let successful = results.iter().filter(|r| r.outcome == crate::experiment::runner::TrialOutcome::Success).count();
        let failed = results.iter().filter(|r| r.outcome == crate::experiment::runner::TrialOutcome::Failure).count();
        let timeouts = results.iter().filter(|r| r.outcome == crate::experiment::runner::TrialOutcome::Timeout).count();
        println!("Experiment complete: {successful} successful, {failed} failed, {timeouts} timeout of {} total", results.len());
        return Ok(());
    }

    Err(miette!("could not parse the prepared manifest (expected first-rocket or starter-modules format)"))
}
...
fn run_report(args: &ArgMatches) -> Result<()> {
    ...
    if is_first_rocket {
        ...
        println!("First-rocket report written to {}", output_dir.join("report.md").display());
    } else {
...
fn load_results_from_runs(runs_dir: &std::path::Path) -> Result<Vec<crate::experiment::runner::TrialResult>> {
            ...
            if path.is_dir() {
                ...
                if result_path.exists() {
                    if let Ok(content) = std::fs::read_to_string(&result_path) {
                        if let Ok(result) = factorio_bot_core::serde_json::from_str::<crate::experiment::runner::TrialResult>(&content) {
                            results.push(result);
                        }
                    }
                }
