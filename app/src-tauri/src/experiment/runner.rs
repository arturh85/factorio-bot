...
use crate::experiment::manifest::{ManifestError, ResolvedManifest, ResolvedFirstRocketManifest, TrialKey};
...
impl TrialResult {
    ...
    pub fn is_invalid_zero_time(&self) -> bool {
        matches!(self.outcome, TrialOutcome::Success)
            && self.tick_duration().map_or(true, |d| d == 0)
    }
}
...
fn run_single_live_trial(
    ...
    Ok(TrialResult {
        ...
        wall_ms: elapsed,
        reason: None,
        policy_hash: Some(hash_string(&format!("{}-{:?}", key.variant, key.repetition))),
...
pub fn run_offline_trial(
    ...
    let result = factorio_bot_planner::plan_best(
        &[goal],
        &state,
        &registry,
        chain_actor,
...
mod tests {
    ...
    fn fake_process_runner_produces_one_row_per_declared_trial() {
        ...
        let keys = crate::experiment::manifest::expand_first_rocket_trials(&rocket_manifest, true).unwrap();
        assert_eq!(keys.len(), 6, "expected 6 trial keys from first-rocket manifest");
