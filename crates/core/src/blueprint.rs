...
//!
//! **This decoder refuses by allowlist, not by blocklist.** An entity is
//! understood only if every key it carries is one of `entity_number`,
//! `name`, `position`, `direction`, `type` -- anything else (`items`,
//! `recipe`, `filters`, `connections`, circuit wiring, or a Factorio feature
//! that does not exist yet) fails the whole decode with
...
//! fixtures, so a blocklist would have looked correct while being wrong. A
//! block that quietly builds two thirds of itself is worse than one that
//! refuses loudly, so deny-by-default is the promise here: every refusal
//! names the exact key that tripped it, and there is no path that drops a
//! key silently.
...
pub fn decode(text: &str) -> Result<Blueprint, BlueprintError> {
    ...
    let envelope: Envelope = serde_json::from_slice(&json).map_err(|_| BlueprintError::NotJson)?;
