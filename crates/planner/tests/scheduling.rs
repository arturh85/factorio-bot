...
fn insert_at(gen: &mut ActionIdGen, pos: &Position) -> Action {
    Action {
        id: gen.next(),
...
fn two_bots_cannot_mine_the_same_exhausted_tile() {
    ...
    let second = net.add(mine_at(&mut gen, &pos, 1));
...
fn two_bots_cannot_place_at_the_same_position() {
    ...
    let second = net.add(place_at(&mut gen, &pos));
...
fn the_property_still_holds_across_an_inferred_entity_edge() {
    ...
    let mut gen = ActionIdGen::new();
    let mut net = ActionNetwork::new();
    let place = net.add(place_at(&mut gen, &furnace));
    let insert = net.add(insert_at(&mut gen, &furnace));
    net.infer_edges();
    assert_eq!(
        net.preds(insert),
        vec![(place, 0)],
...
fn the_time_ordered_replay_catches_an_overlapping_schedule() {
    ...
    let mut gen = ActionIdGen::new();
    let mut net = ActionNetwork::new();
    let first = net.add(mine_at(&mut gen, &pos, available));
