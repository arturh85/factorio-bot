...
assert(research.makespan > 0, "expected a positive makespan for the research plan")
assert(research:count { kind = "research" } > 0, "the research action must reach the plan")
-- The packs are actually produced rather than assumed: the same plan has to
-- mine for them.
assert(research:count { kind = "mine" } > 0, "the science packs must be produced by the plan")
