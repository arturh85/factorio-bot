...
## The second customer: "with recipe X open"

`b68bd455` (electric offtake) states its own limit: no archived dump has
`electronics` researched, so its electric-arm leg is exercised only by an
offline fixture and becomes reachable live only on a replan with the plant
already standing. The seam can say that now: `--researched <technology>`
marks a technology researched on the acting force before planning -- a
**stated hypothesis**, never read off a record -- and composes with either
standing world. A world with a plant standing is one command away
(`plan --goal researched:automation --replan 1 --dump-standing plant.json`).

Four plans of `sustain:iron-plate:30:36000`, four bots, one binary
(`2f3a5a05` plus this), offtake arms counted off `--steps`:

| world | `electronics` | actions / makespan | burner arms | electric arms | poles |
|---|---|---:|---:|---:|---:|
| t=0 dump | closed | 1,111 / 62,064 | 20 | 4 | 2 |
| t=0 dump | **open** | 1,097 / 59,712 | 20 | 4 | 2 |
| plant standing | closed | 606 / 32,028 | 20 | 0 | 0 |
| plant standing | **open** | 607 / 34,923 | **14** | **2** | **12** |

**The electric leg fires only in the last row**, and only there: the recipe
open with no network changes nothing (row 2 -- the four electric arms and
the plant in rows 1-2 are the plan's own, built for something else, and the
same in both), the network standing with the recipe closed changes nothing
(row 3), and both together turn two arms electric and lay the twelve poles
`power_offtake` needs to reach them. That is the guard its author described
-- recipe `Open` *and* a standing network *and* a pole run that reaches --
observed rather than asserted, with no run spent.

**What the record-driven path could NOT show here, and why.** The obvious
world for this -- `run-1788926478-07032` at tick 56,168, which has a plant
standing -- refuses every new cell before the arm decision is visible:
*"nothing can carry coal from the buffer at [16.5,-26.5] to the iron-chest
at [-29.5,-7.5] ... the two machines are further apart than one search
window reaches"*. A new cell planned beside a standing base is fed from the
