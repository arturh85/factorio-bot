...
## Task 13: Delete TypeScript type generation instead of moving it

**An earlier draft of this task moved `typescriptify()` into a new `crates/server/build.rs`, to stop Task 14's removal of the `gui` feature silently killing it. Measured before dispatch, that is the wrong fix: the generator should be deleted, not relocated.**

`app/src-tauri/build.rs` emits **33 types** into `app/src/models/types.ts`. The frontend imports that file in exactly **three places**, for exactly **two symbols**:

    app/src/store/scriptStore.ts:3      PrimeVueTreeNode
    app/src/components/ScriptTree.vue:5 PrimeVueTreeNode
    app/src/models/settings.ts:1        FactorioSettings

The other 31 have no consumer at all — `grep -rln FactorioEntity app/src` returns only `types.ts` itself. And both survivors are superseded by earlier tasks in this plan: `PrimeVueTreeNode` becomes `ScriptTreeNode` in Task 2 and arrives through the spec-derived DTOs of Task 4, and `FactorioSettings` is what `GET /api/v1/settings` returns, also a DTO.

So after Task 4 there are **three** representations of one set of types: the Rust structs, the utoipa OpenAPI spec derived from them, and a second Rust→TypeScript path derived from them independently. The second path has two consumers, both of which the first path already serves.

The rule this follows, and the reason it is a deletion rather than a move: **if two places decide the same thing, the bug is the second place existing.** Synchronising them — or relocating one so it keeps running — moves the failure to whoever edits one of them next. Task 4's DTOs are pinned to the spec by Task 5's contract test, so the remaining single path is checked; a second path checked by nothing is what produced the stale `types.ts` this plan was originally written to preserve.

- [ ] **Step 1: Confirm the measurement still holds.** `grep -rn "from '@/models/types'" app/src` must return only those three imports, and both symbols must already be available from `@/api/types` after Tasks 2 and 4. If a fourth consumer has appeared, stop and report rather than deleting something in use.
- [ ] **Step 2: Repoint the three imports** at the DTOs from Task 4.
- [ ] **Step 3: Delete `app/src/models/types.ts`**, the `typescriptify()` function, its call in `main()`, and the `typescript-definitions` and `factorio-bot-core` entries from `[build-dependencies]` in `app/src-tauri/Cargo.toml`.
- [ ] **Step 4: `pnpm run lint`** — `tsc` and `vue-tsc` are the check that no other consumer existed. A missing import is a compile error, which is the strongest available evidence here.
- [ ] **Step 5: Confirm `typescript-definitions` leaves the build.** `cargo tree -p factorio-bot | grep typescript-definitions` must return nothing. It is a forked, unmaintained crate (`typescript-definitions-ufo-patch`); dropping it is a real reduction, not just tidying.
- [ ] **Step 6: Commit.**

**This removes the ordering hazard rather than managing it.** The original Task 13 existed because generation ran under `#[cfg(feature = "gui")]` with its output committed, so removing the feature would have left a stale file and a green build. Deleting the generator means there is no output to go stale and no ordering constraint against Task 14.

**`docs/lua/src/types.lua` is the same shape and stays for now.** It is tracked, hand-written, and mirrors `crates/core/src/types.rs` by hand — a third such mirror, with no consumer that would break if it drifted. Out of scope here; recorded in the follow-up section.
