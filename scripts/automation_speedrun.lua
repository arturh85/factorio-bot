-- The TIMED variant of `research_run.lua`: one goal, not a seven-rung ladder.
--
-- WHY THIS EXISTS, AND WHEN NOT TO USE IT
--
-- `research_run.lua` climbs seven rungs on purpose, so that a failure names
-- which step broke rather than "research did not happen". That is the right
-- shape for diagnosis and the wrong shape for a stopwatch: measured on
-- `run-1788479942-45523`, the ladder planned ~45,984 ticks against 30,077 for
-- the same end state expressed as ONE goal. Each rung is planned
-- independently and cannot overlap the next, so the ladder pays roughly 53%
-- for its legibility.
--
-- Use `research_run.lua` when something is broken. Use this when the question
-- is "how fast".
--
-- The executor is not the difference. On that same run, execution tracked the
-- plan to within 7.2% overall (milestone 6: 16,894 planned / 17,700 actual;
-- milestone 7: 17,307 / 18,104), so what this script removes is planning
-- structure, not execution slack.
