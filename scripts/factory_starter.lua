-- THE STARTER FACTORY: red AND green science, in one run.
--
-- Four rungs. Red's cell and its witness, then green's cell and its witness.
-- Red first because it is the cheap half and it stands up the power plant,
-- the lab and the research that green then inherits -- measured, not assumed:
--
--   producing red only                    30,077 ticks planned
--   producing green only                 191,105
--   producing BOTH                       190,654   <-- cheaper than green alone
--
-- Red is very nearly free once green's infrastructure has to exist anyway.
-- That is why this is one script rather than two runs.
--
-- WHY max_iterations IS 25 AND NOT 10
--
-- Stage 3's green run halted `exhausted` at 10 iterations with its step count
-- still falling: 456, 465, 392, 330, 294, 285. It was converging, not stuck.
-- The cap was the binding constraint, so it is raised here. If this run also
-- exhausts, the cap is NOT the answer and the furnace reuse gap is -- see
-- below.
--
-- THE KNOWN DISEASE, UNTREATED
--
-- `smelt_steps` calls `commit_machine` on the furnace it uses and the
-- commitment is never released, so a plan needs one furnace per Smelt goal:
-- 28 furnaces for 276 ore in the stage 3 run. Cross-plan reuse works, so each
-- epoch adds ~13 rather than starting over, but it never converges to reuse.
-- `3ba0d441` reserves six cell sites so this can no longer strand the ore cell
-- that feeds it, which is the symptom that halted runs -- the disease itself
-- needs a scheduler change that would move red, and is its own work.
--
-- THE WITNESSES ARE THE ONLY EVIDENCE ABOUT PRODUCTION.
-- Rungs 1 and 3 are structural: a cell whose chests are empty satisfies them
-- exactly as a working one does, because `PlanState` reads no container
-- contents and no fuel level. Only rungs 2 and 4, which dispatch nothing and
-- watch output inventories while every bot stands still, distinguish a cell
-- that stands from a cell that produces.

-- Stage 3 of the starter factory: GREEN science, against a live game.
--
-- Two rungs, not three. `producing("logistic-science-pack", 6)` drags its own
-- prerequisites behind it -- the research, and therefore a lab, and therefore
-- a power plant -- so there is no separate `researched` rung to name. Stage 2
-- needed one because `assembling-machine-1` is locked until `automation` and a
-- failure there had to be distinguishable from a failure in the cell.
--
-- WHAT MAKES GREEN DIFFERENT FROM RED
--
-- Red is `copper-plate` + `iron-gear-wheel`: one ingredient the cell can make,
-- one it cannot. That is exactly the shape `assembly_spec` was built for.
--
-- Green is `transport-belt` + `inserter` and BOTH are craftable, and `inserter`
-- takes three ingredients of its own. The cell that resulted (240d3efb) widens
-- the intermediate machine to two mouths rather than adding a second machine --
-- forced, not preferred: the intermediate presents three tiles to its west and
-- one small pole lights two of them.
--
-- So `transport-belt` is made in-cell and WHOLE INSERTERS ARE CHEST-FED,
-- hand-crafted by a bot: twelve per cell per charge, each dragging a circuit,
-- a gear and a plate behind it. This is a first green cell, not a green
-- factory. Expect the bill to dominate the run.
--
-- WHAT THIS COSTS, PLANNED
--
-- 730 actions, makespan 191,105 ticks (53:05) on the reference map, against
-- 30,077 (8:21) for `researched("automation")`. Two cells are planned, not
-- one, each bringing its own pole, and the lab brings a third (38869772).
--
-- THE WITNESS IS STILL THE ONLY EVIDENCE ABOUT PRODUCTION.
-- Everything else here is structural. A cell whose chests are empty satisfies
-- rung 1 exactly as a working one does -- `PlanState` reads no container
-- contents and no fuel level. Only the witness, which dispatches no actions
-- and watches output inventories while every bot stands still, distinguishes
-- a cell that stands from a cell that produces.
