...
-- ---------------------------------------------------------------- GHOSTS --
-- Task 8's live proof. Two things this checks that nothing else can:
--   1. the plan actually EMITTED a stamp (a ghost path nobody dispatches is
--      inert, and a green suite would not notice);
--   2. no ghost of this block SURVIVES the build -- measured earlier as
--      consumed by the real placement, and the whole viewer benefit depends
--      on it, because leftovers make a finished block look unfinished.
print("")
print("---- ghosts ----")
local stamped = 0
for _, s in ipairs(plan.steps) do
  if s.kind == "stamp_ghosts" or (s.entity == nil and tostring(s.kind):find("ghost")) then
    stamped = stamped + 1
    print("STAMP step: kind=" .. tostring(s.kind) .. " pos=" .. tostring(s.pos and s.pos.x))
  end
end
print("stamp steps in the plan: " .. stamped .. " (want exactly 1 on a fresh site)")

local leftover = 0
local names = {"transport-belt","inserter","stone-furnace","small-electric-pole",
               "small-lamp","splitter","underground-belt"}
for _, n in ipairs(names) do
  local found = rcon.find_entities_in_radius(footprint_center, 21, "entity-ghost")
  if type(found) == "table" then
    for _, g in ipairs(found) do
      if g.ghost_name == n then leftover = leftover + 1 end
    end
  end
  break
end
print("GHOSTS SURVIVING THE BUILD: " .. leftover .. " (want 0 -- a real placement consumes the ghost beneath it)")
if leftover > 0 then
  print("LEFTOVER GHOSTS: a finished block will look unfinished in the viewer")
end
