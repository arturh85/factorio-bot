-- The first rung of a STANDING goal, against a live game.
--
-- Design note: `docs/superpowers/notes/2026-09-06-standing-goals.md`.
--
-- THREE rungs, and the third is the new one. Rung 1 builds the cell
-- (`goal.producing`, capacity). Rung 2 is the existing witness -- 1 plate into
-- a fed furnace's output with every bot idle, which proves the machines are
-- RUNNING. Rung 3 is `supervisor.sustain`, and it exists because rung 2 cannot
-- distinguish a fed factory from a charged one: a stone furnace's input slot
-- holds one stack of 50 ore at 3.2 s a plate -- 9,600 ticks of hand-fed
-- running -- so the witness's window sits comfortably inside a single hand
-- load.
--
-- Rung 3 dispatches nothing for LEAD_IN + WINDOW ticks and reaches no verdict
-- about the rate: the measurement is over per-machine lifetime counters, which
-- ride on the sampling stream into `samples.jsonl` and are not on the entities
-- a script can read. It establishes the conditions instead -- an idle roster
-- across the window and the lead-in before it -- and the answer comes from
--
--     tools/run_analysis.py --sustain iron-plate:15:7200:9600 <run-dir>
--
-- **This run is EXPECTED to fail that check**, and the failure is the finding:
-- a burner cell is fed by hand, one coal burns ~1,600 ticks in a drill and
-- ~2,666 in a furnace, and `method::connect` -- the belt primitive that would
-- deliver coal without a bot -- still has no caller. What the run measures is
-- how far the factory is from feeding itself, which nothing has measured yet.
...
local SUSTAIN_LEAD_IN_TICKS = 9600
