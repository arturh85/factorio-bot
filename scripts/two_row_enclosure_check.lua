...
print("start two row enclosure check")
...
-- THE ORIGINAL FAILING CONFIGURATION, on purpose: no site at all, so the block
-- is sited from the roster's own seed and lands around the bots. Before the
-- enclosure guard this produced `the character is already walled in here ...
-- pocket_tiles=1.0`, a walk ending inside a furnace's collision box, and
-- done=true beside pending=13 with 2 of 6 furnaces standing.
--
-- If the guard works, the bots walk clear first and the block completes.
local plan = goal.plan(goal.built(BP))
local evac = 0
for _, st in ipairs(plan.steps) do
  if st.kind == "act" and st.label and st.label:find("evacuate") then evac = evac + 1 end
end
print("evacuation steps planned: " .. evac)
...
print("end two row enclosure check")
